...
export const Route = createFileRoute("/experience")({
  head: () => ({
    meta: [
      { title: "Experience — Advocate Profile" },
      { name: "description", content: "Internships, legal aid, and research work at the edge of practice." },
      { property: "og:title", content: "Experience" },
      { property: "og:description", content: "Internships, legal aid, and research work." },
    ],
  }),
  ...
  return (
    ...
    >
      <div className="space-y-0">
        {ROLES.map((r) => (
          <article key={r.role + r.period} className="grid grid-cols-1 gap-6 border-t border-border py-12 md:grid-cols-12 md:gap-8">
            ...
            <div className="md:col-span-3">
              ...
              <ul className="mt-3 space-y-1 text-sm">
                {r.skills.map((s) => (
                  <li key={s} className="border-b border-border pb-1">{s}</li>
                ))}
              </ul>
            </div>
          </article>
